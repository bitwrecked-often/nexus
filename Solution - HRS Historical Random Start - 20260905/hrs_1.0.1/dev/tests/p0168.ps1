...
try {
    ...
    Invoke-HrsContractCase 'POLICY-002A-random-safe-construction' {
        $policy = New-HrsPolicy -Revision ([UInt64]3) -GameName 'Safe Snow' -Mode RandomSafe -WrittenUtc $fixedUtc
        Assert-HrsTrue $policy.enabled 'RandomSafe did not enable relocation.'
        Assert-HrsEqual $policy.mode 'RandomSafe' 'RandomSafe mode changed.'
        $roundtrip = ConvertFrom-HrsPolicyJson -Json (ConvertTo-HrsPolicyJson -Policy $policy)
        Assert-HrsEqual $roundtrip.mode 'RandomSafe' 'RandomSafe failed canonical roundtrip.'
    }
