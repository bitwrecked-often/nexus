...
## 2026-09-01 — Primed landing release candidate

- Promoted the carousel-proven two-second priming delay, elevated second
  placement, and actual grounded-surface verification into the 1.1.0 release
  transaction.
- Kept the production random pool restricted to NG01 and NG02; no survey or
  full-catalog point became selectable from this change.
- Replaced preloaded tallest-block height use with catalog elevation for the
  first anchor and loaded-chunk terrain height for the final landing search.
- Preserved exactly-once reservation/completion, Standard bypass, failure
  restoration, snow handling, starter quest, and nearest-trader routing.
- Updated the current game-build MVID pin across runtime, build, manager,
  manifest, and tests.
- Deterministically built and statically verified the 39,936-byte candidate
  `DA32165FA443C3FF17FA7902B851FF437F1892BBA395D22F39167291F0DF0507`.
- Passed release source, manager, pure decision, marker, relocation, semantic,
  preview, core, deployment, and management contracts. OneDrive reparse-point
  fixture failures passed unchanged in an ordinary isolated filesystem.
- Left packaging/release blocked on fresh no-God-mode NG01 and NG02 live QA.
