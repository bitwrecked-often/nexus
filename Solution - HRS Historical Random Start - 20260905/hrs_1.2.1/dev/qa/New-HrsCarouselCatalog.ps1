...
foreach ($pointId in @($AdditionalExcludedPointIds)) {
    if ($pointId -notmatch '^NVG-\d{4}$') {
        throw 'Additional carousel exclusion contains an invalid point ID.'
    }
    $rejected[$pointId] = $true
}
