...
Assert-Count 'AddCustomVar\s*\(' 8 'marker/biome/protection/trader-route write call'
...
Assert-Count 'new CuratedStartPoint\("NVG-\d{4}"' 171 'certified NVG landing point'
...
$approvalMatch = [regex]::Match($catalog,
    'private static readonly int\[\] ApprovedPointIndices\s*=\s*\{(?<body>.*?)\};',
    [System.Text.RegularExpressions.RegexOptions]::Singleline)
if (-not $approvalMatch.Success) {
    throw 'Approved production selection pool is missing.'
}
$approvedIndices = @([regex]::Matches($approvalMatch.Groups['body'].Value, '\d+') | ForEach-Object { [int]$_.Value })
$expectedApprovedIndices = @(
...
if ($approvedIndices.Count -ne 80 -or
    ($approvedIndices -join ',') -cne ($expectedApprovedIndices -join ',')) {
    throw 'Approved production selection pool is not exactly the curated balanced 80-point set.'
}
foreach ($required in @(
...
foreach ($required in @('ProtectArrivalBiome', 'bitwrecked_hrs_biome_v1',
    'bitwrecked_hrs_initial_biome_v1', 'TrySetInitialBiome',
    'TryEnableProtection', 'ReadProtectionFamily', 'world.GetBiome(',
    '"buff" + prefix + "_Hazard"', '"$SnowHazardTimerMax"',
    '"$WastelandHazardTimerMax"', '"$DesertHazardTimerMax"',
    '"$BurntHazardTimerMax"', 'int[] biomes = { 5, 1, 9, 8, 3 }',
    'if (biomes[i] != initialBiome) continue;')) {
    if (-not $text.Contains($required)) {
        throw "Arrival-biome protection requirement missing: $required"
    }
}
...
Write-Output 'HRS_RELEASE_SOURCE_STATIC_SUMMARY pass=1 fail=0 csFiles=11 surveyPoints=12 approvedPoints=80 selectCalls=1 placeCalls=1 resultAuthorityReads=0'
