...
$stageRoot = Join-Path $projectRoot 'tmp'
...
$expectedHarmonySha256 = 'C349E1A3FD13FA5A9FACC9805A5E160161B14489F46F6BDD38202B8E124F78DF'
$expectedHarmonyMvid = [guid]'7b787fec-e12a-47a6-8027-108671090713'
...
$tfpHarmony = Join-Path $game 'Mods\0_TFP_Harmony\0Harmony.dll'
$references = @(
    'mscorlib.dll',
    'netstandard.dll',
    'System.dll',
...
$references = @($references) + @($tfpHarmony)
...
if ((Get-Sha256 $tfpHarmony) -cne $expectedHarmonySha256) { throw 'TFP Harmony hash changed.' }
...
$harmonyAssembly = [System.Reflection.Assembly]::ReflectionOnlyLoadFrom($tfpHarmony)
if ($harmonyAssembly.ManifestModule.ModuleVersionId -ne $expectedHarmonyMvid) { throw 'TFP Harmony MVID changed.' }
...
$payloadRoot = Join-Path $candidateRoot 'mod'
...
$recordPath = Join-Path $candidateRoot 'j0144.json'
