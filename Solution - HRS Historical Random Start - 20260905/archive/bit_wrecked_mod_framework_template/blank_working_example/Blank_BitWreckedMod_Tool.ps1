# Bit Wrecked Mod Framework Design Tool - empty framework shell.
...
# layout. It has no feature rows, no payload, and no install/remove capability.
...
$form.Text = 'Bit Wrecked - 7DTD Mod Framework'
...
$versionLabel.Text = 'Reusable framework | Version 0.0.1'
...
$headers = @(
    @{ Text = 'Feature Group'; Left = 22; Width = 118 },
...
$emptyState.Text = "No mod features defined yet.`r`n`r`nApproved feature groups and mod rows will appear here."
...
$activityLog.Text = "[$(Get-Date -Format 'HH:mm:ss')] Ready.$([Environment]::NewLine)[$(Get-Date -Format 'HH:mm:ss')] DESIGN | Empty shell; no feature rows or payload are configured.$([Environment]::NewLine)"
...
function Update-DesignState {
    if (Test-GameRoot -GameRoot $pathBox.Text) {
        ...
        $stateValue.Text = 'Game folder recognized - no mod payload is configured.'
...
Set-Tip -Control $designPanel -Text 'This deliberately empty table is where approved feature groups and mod rows will appear later.'
...
$scanButton.Add_Click({
    if (Test-GameRoot -GameRoot $pathBox.Text) {
        [System.Windows.Forms.MessageBox]::Show(
            "Game folder recognized.`n`nNo approved line items exist yet.`nNo payload, installer, or game-file write action is enabled.",
            'Mod Framework Design Workspace',
            'OK',
            'Information'
        ) | Out-Null
    }
...
$openFolderButton.Add_Click({
    if (-not (Test-GameRoot -GameRoot $pathBox.Text)) {
        [System.Windows.Forms.MessageBox]::Show('Choose a valid 7 Days to Die game folder first.', 'Mod Framework Design Workspace', 'OK', 'Warning') | Out-Null
    ...
    if (-not (Test-Path -LiteralPath $modsPath -PathType Container)) {
        [System.Windows.Forms.MessageBox]::Show('No Mods folder exists. This design shell will not create one.', 'Mod Framework Design Workspace', 'OK', 'Information') | Out-Null
...
$chooseLogFileButton.Add_Click({
    ...
    $dialog.FileName = "BitWrecked-ModFramework-Design-$(Get-Date -Format 'yyyyMMdd-HHmmss').txt"
...
$persistentLogCheck.Add_CheckedChanged({
    ...
    if ($persistentLogCheck.Checked) {
        if ([string]::IsNullOrWhiteSpace($script:PersistentLogPath)) {
            ...
            $script:IsUpdatingPersistentLog = $false
            [System.Windows.Forms.MessageBox]::Show('Choose a log file first. Runtime logging remains active and is not saved automatically.', 'Mod Framework Design Workspace', 'OK', 'Information') | Out-Null
            return
        }
...
$form.Add_Shown({
    ...
    Add-Activity -Message 'Design workspace started. No feature rows, payload, installer, removal, or game-file write action is available.' -Color (New-Color 82 82 78)
    if ($SmokeTest) {
