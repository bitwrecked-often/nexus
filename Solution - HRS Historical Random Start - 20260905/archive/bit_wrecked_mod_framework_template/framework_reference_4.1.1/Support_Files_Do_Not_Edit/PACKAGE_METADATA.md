...
- Documentation baseline date: 2026-07-18
...
Tune Wasteland animal density and mix through generated modlet XML. The optional
global-cap action separately backs up and writes `serverconfig.xml`; its restore
action replaces that file from the newest matching backup.
...
Default installed should match the live Wasteland animal baseline values while
still writing generated mod XML. Remove Mod deletes this tool's mod folder, so
the affected routes return to the game's effective default XML configuration;
it does not restore `serverconfig.xml`.
...
```text
Be transparent and welcoming. Mention the current known review notes: Default
precision was fixed but deserves clean gameplay confirmation, Dense/Absurd can
be limited by the global animal cap, the global-limit action backs up and writes
serverconfig.xml, its restore action replaces that file from the newest matching
backup, Remove Mod deletes only this tool's mod folder and does not restore the
cap, Vortex users may prefer the modlet folder only, and screenshot examples are
still being curated. Ask users to report tuning level, global-limit state,
single-player/server, other spawn/gameplay mods, and expected versus observed
behavior. This is a community mod, not a competition.
```
