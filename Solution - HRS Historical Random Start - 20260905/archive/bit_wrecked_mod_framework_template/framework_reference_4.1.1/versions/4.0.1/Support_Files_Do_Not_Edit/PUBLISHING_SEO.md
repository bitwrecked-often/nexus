...
- Brutal Science can raise MaxSpawnedAnimals in serverconfig.xml to 999 after making a timestamped backup. Use Restore Cap to restore from the newest Bit Wrecked backup.
- Remove Mod removes the modlet folder. Restore Cap is the separate undo path for the optional serverconfig.xml animal-cap change.
...
The safest path is Remove Mod. That means the game is left alone.

Default means the mod writes the current game baseline back into the modlet. It is useful when you want the tool installed but do not want the animal mix changed. Remove Mod is the clean untouched comparison state.
...
Brutal Science is different. It does not create animals by itself. It lifts the global animal cap in serverconfig.xml so Dense and Absurd are less likely to be stopped by the world's safety rail. Vanilla despawn and cleanup behavior still exists, but high route pressure plus a lifted cap can get heavy over time. The tool makes a backup first, and Restore Cap can put the newest Bit Wrecked backup back. Treat this like opening a lab door: useful, loud, and not always polite to your hardware.
...
Brick 0.6: clarified the testing baseline. Remove Mod is the clean untouched game state. Default installed writes the live game baseline values through the modlet and should feel like no mod for the tuned Wasteland animal routes.
...
Brick 3: built clear tuning levels: Absent, Sparse, Default, Dense, Absurd. Default is the live game baseline. Remove Mod is the true untouched vanilla state.
...
Brick 6: added Brutal Science animal cap support. This optional setting backs up serverconfig.xml and can raise MaxSpawnedAnimals to 999 for extreme testing. It is clearly marked as dangerous because it can stress hardware, servers, saves, and good judgment. The tool also includes Restore Cap, which restores serverconfig.xml from the newest Bit Wrecked animal-cap backup.
