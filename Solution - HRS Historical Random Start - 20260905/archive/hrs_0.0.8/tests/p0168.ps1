...
$modulePath = Join-Path $projectRoot 'src\launcher\m0161.psm1'
...
try {
    ...
    Invoke-HrsContractCase 'BRIDGE-001-path-layout-is-bounded' {
        ...
        Assert-HrsEqual $paths.ReleaseRoot (Join-Path $bridgeGame 'Mods\mod') 'Release root differed.'
        Assert-HrsEqual $paths.PolicyPath (Join-Path $bridgeGame 'Mods\mod\Bridge\policy.v1.json') 'Policy path differed.'
        Assert-HrsEqual $paths.ResultPath (Join-Path $bridgeGame 'Mods\mod\Bridge\result.v1.json') 'Result path differed.'
    }
    ...
    Invoke-HrsContractCase 'BRIDGE-004-existing-owned-layout-valid' {
        [void] [System.IO.Directory]::CreateDirectory((Join-Path $bridgeGame 'Mods\mod\Bridge'))
        $actual = Test-HrsBridgePaths -GameRoot $bridgeGame -RequireReleaseRoot
    ...
    Invoke-HrsContractCase 'BRIDGE-004A-unknown-bridge-entry-rejected' {
        $unknown = Join-Path $bridgeGame 'Mods\mod\Bridge\unknown.bin'
    ...
    if ($junctionSupported) {
        ...
        Invoke-HrsContractCase 'BRIDGE-008-bridge-directory-reparse-rejected' {
            ...
            $release = Join-Path $game 'Mods\mod'
    ...
    Invoke-HrsContractCase 'MANAGER-001-state-path-layout' {
        ...
        Assert-HrsEqual $paths.StateRoot (Join-Path $managerRoot 'ui\HistoricalRandomStart_State') 'Manager state root differed.'
    ...
    [void] [System.IO.Directory]::CreateDirectory((Join-Path $managerRoot 'ui'))
    ...
    Invoke-HrsContractCase 'HISTORY-002-all-action-categories-append' {
        ...
        [void] [System.IO.Directory]::CreateDirectory((Join-Path $root 'ui'))
