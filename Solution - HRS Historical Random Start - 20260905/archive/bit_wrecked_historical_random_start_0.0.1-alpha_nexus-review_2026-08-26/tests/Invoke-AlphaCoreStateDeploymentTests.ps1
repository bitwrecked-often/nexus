...
$corePath = Join-Path $launcherRoot 'HistoricalRandomStart.Core.psm1'
$statePath = Join-Path $launcherRoot 'HistoricalRandomStart.State.psm1'
$copyPath = Join-Path $launcherRoot 'HistoricalRandomStart.Copy.psm1'
$deploymentPath = Join-Path $launcherRoot 'HistoricalRandomStart.Deployment.psm1'
...
function New-HrsDeploymentFixture {
    ...
    [System.IO.File]::WriteAllBytes((Join-Path $payloadRoot 'HistoricalRandomStart.dll'), [byte[]](1..32))
    ...
    try {
        ...
        $dllPath = Join-Path $payloadRoot 'HistoricalRandomStart.dll'
        ...
        $manifest = [pscustomobject][ordered]@{
            ...
            files = @(
                [pscustomobject][ordered]@{ path='HistoricalRandomStart.dll'; bytes=[UInt64]$dllInfo.Length; sha256=Get-HrsFixtureHash $dllPath },
    ...
    if ($Installed) {
        ...
        [System.IO.File]::Copy((Join-Path $payloadRoot 'HistoricalRandomStart.dll'), (Join-Path $paths.ReleaseRoot 'HistoricalRandomStart.dll'))
...
try {
    ...
    Invoke-HrsCase 'COPY-001-catalog-self-validation' {
        ...
        Assert-HrsEqual $actual.Count 16 'Copy catalog count differed.'
    }
    Invoke-HrsCase 'COPY-002-required-keys-exactly-once' {
        $items = @(Get-HrsCopyCatalog)
        $required = @('GameName','Standard','Random','Apply','LaunchGame','Recovery','ShowChanges','RestoreLast','RestoreDefault','RemoveFromGame','StatusIncomplete','StatusStandard','StatusRandom','StatusRunning','StatusConflict','StatusLane')
    ...
    Invoke-HrsCase 'DEPLOY-MANIFEST-003-static-files-required' {
        ...
        Assert-HrsThrows { New-HrsDeploymentManifest -PayloadRoot $root -ReleaseVersion '1.0.0' } 'DEPLOYMENT_FILE_MISSING_HISTORICALRANDOMSTART_DLL'
        [System.IO.File]::WriteAllBytes((Join-Path $root 'HistoricalRandomStart.dll'), [byte[]]@(1))
    ...
    Invoke-HrsCase 'DEPLOY-INVENTORY-007-static-drift' {
        ...
        [System.IO.File]::WriteAllBytes((Join-Path $fixture.Paths.ReleaseRoot 'HistoricalRandomStart.dll'), [byte[]]@(9,9,9))
        $actual = Test-HrsDeploymentInventory -GameRoot $fixture.GameRoot -Manifest $fixture.Manifest
    ...
    if ($junctionSupported) {
        Invoke-HrsCase 'DEPLOY-INVENTORY-011-release-root-reparse-rejected' {
            ...
            [System.IO.File]::Copy((Join-Path $fixture.PayloadRoot 'HistoricalRandomStart.dll'), (Join-Path $target 'HistoricalRandomStart.dll'))
    ...
    Invoke-HrsCase 'REMOVE-004-exact-capsule-only-manager-retained' {
        ...
        $managerSupport = Join-Path $manager 'Support_Files_Do_Not_Edit\HistoricalRandomStart_State'
        [void] [System.IO.Directory]::CreateDirectory($managerSupport)
        Write-HrsFixtureText -Path (Join-Path $manager '7DTD_HistoricalRandomStart.bat') -Text 'portable manager'
