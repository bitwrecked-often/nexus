...
$sourceRoot = Join-Path $PSScriptRoot '..\src\runtime\BitWrecked.HistoricalRandomStart.Phase1A.RelocationProbe'
...
Assert-CallCount 'Time\.realtimeSinceStartup' 2 'monotonic realtime sampling'
Assert-CallCount 'SemanticSnapshot\.Capture\s*\(' 2 'atomic semantic capture'

$semantic = Get-Content -LiteralPath (Join-Path $sourceRoot 'SemanticSnapshot.cs') -Raw
...
$handler = Get-Content -LiteralPath (Join-Path $sourceRoot 'RelocationModApi.cs') -Raw
...
if (-not (0 -le $reserve -and $reserve -lt $select -and $select -lt $defer -and $defer -lt $semanticBefore -and
    ...
    $safe -lt $distance -and $distance -lt $complete)) {
