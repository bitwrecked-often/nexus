...
$sourceRoot = Join-Path $PSScriptRoot '..\src\runtime\BitWrecked.HistoricalRandomStart.Phase1A.MarkerProbe'
...
$handler = Get-Content -LiteralPath (Join-Path $sourceRoot 'MarkerProbeModApi.cs') -Raw
...
$marker = Get-Content -LiteralPath (Join-Path $sourceRoot 'MarkerState.cs') -Raw
...
if ($modInfo.xml.Name.value -ne 'BitWrecked_HistoricalRandomStart_PHASE1A_DEV') {
