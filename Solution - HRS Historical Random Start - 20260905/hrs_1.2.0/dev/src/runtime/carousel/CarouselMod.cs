    ...
    internal sealed class CarouselRun
    {
        ...
        private bool TickDwell(EntityPlayer player)
        {
            ...
            CarouselLog.Event(paths, arm, point.Id, "POINT_PASS", anchor,
                player.GetPosition(), Health(), arm.DwellSeconds >= 210
                    ? "HAZARD_MASK_" + HazardMask(player).ToString(
        ...
        private static int HazardMask(EntityPlayer player)
        {
            int mask = 0;
            if (FamilyActive(player, "Burnt")) mask |= 1;
            if (FamilyActive(player, "Desert")) mask |= 2;
            if (FamilyActive(player, "Wasteland")) mask |= 4;
            if (FamilyActive(player, "Snow")) mask |= 8;
            return mask;
        }

        private static bool FamilyActive(EntityPlayer player, string prefix)
        {
            string[] names =
            {
                "buff" + prefix + "_Hazard",
            ...
            for (int i = 0; i < names.Length; i++)
                if (player.Buffs.HasBuff(names[i])) return true;
            return false;
        }
