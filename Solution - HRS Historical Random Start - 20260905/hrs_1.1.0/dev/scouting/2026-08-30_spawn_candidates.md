...
## 2026-08-30/31 carousel evidence

- Run `2687e1884ac64a9aac1967e8684cc787` on `HRS_CAROUSEL_110_001`:
  NG01 and NG02 passed grounded placement, unchanged health, and a 30-second
  dwell.
- NG03 initially failed closed with `CHUNK_TIMEOUT`; the runtime restored the
  exact captured origin and stopped. A later full rerun passed NG03, showing
  that the first timeout was transient rather than a repeatable point defect.
- Run `347a1c3ac04248beae88de5bafd71244` resumed at NG04 with a
  five-second dwell. NG04 through NG12 passed 9/9 with health `100`, and the
  runtime restored the exact captured origin.
- Recorded repeat `c35a7deae9c749e88c275050f8b47f7d` visually captured NG04
  through NG12 passing 9/9 at five seconds, health `100`, and exact origin
  restoration. The committed video manifest binds the local MP4 to its exact
  event and result hashes.
- Full rerun `818222bdaf094acda7d32c9bdbdc02f8` passed NG01 through NG04,
  including NG03, then NG05 failed closed with `SETTLE_FAILED`. Its landing
  moved from Y `62` to Y `51`, indicating unstable placement. The runtime
  restored the exact captured origin and stopped. NG05 is rejected from the
  current candidate set pending a new anchor or landing design.
- Resume run `77cf8b4d9be64133acc8ef5b4e7669a1` encountered health `0` before
  NG06 teleport and stopped with `HEALTH_INVALID`. This is retained as setup
  evidence and is not an NG06 location failure.
- Resume run `b6b5c11b2bf441e8847cda512e091b8f` passed NG06 through NG12
  7/7 at five seconds with health `100`, then restored the exact origin.
- Consolidated placement-health result: 11/12 anchors passed. NG01-NG04 and
  NG06-NG12 pass this DEV carousel check; NG05 is the only rejected point.
  This does not promote any survey anchor into the production selector.
- The next screening set retires NG05 without renumbering, adds NG13-NG41,
  and keeps 40 active candidates balanced at eight per biome. Its catalog and
  batch ranges are in `2026-08-31_carousel_40_catalog.md`.
