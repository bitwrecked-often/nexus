...
$before = Get-Content (Join-Path $PSScriptRoot 'fixtures\phase1a_semantic_before.valid.json') -Raw | ConvertFrom-Json
$after = Get-Content (Join-Path $PSScriptRoot 'fixtures\phase1a_semantic_after.valid.json') -Raw | ConvertFrom-Json
$pass = 0
$fail = 0
...
$changed = Get-Content (Join-Path $PSScriptRoot 'fixtures\phase1a_semantic_after.valid.json') -Raw | ConvertFrom-Json
...
$digestOne = Get-Phase1ATreeDigest -Root $fixtureRoot -ExcludedRelativePrefixes @('phase0a_')
$digestTwo = Get-Phase1ATreeDigest -Root $fixtureRoot -ExcludedRelativePrefixes @('phase0a_')
