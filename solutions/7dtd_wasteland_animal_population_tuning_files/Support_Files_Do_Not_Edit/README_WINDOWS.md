...
`Remove Mod` is different. It deletes the Bit Wrecked mod folder and leaves the game to use its own files directly. Use Remove Mod for the clean untouched comparison.
...
4. Look at `Spawn tuning`.
5. Leave `All animals` off for no changes.
6. Check `All animals` to tune everything together.
...
11. Click `Scan Values` any time to compare current and selected values.
...
Unchecked animals stay at game values.

The tool shows day and night XML numbers before install and verifies the written numbers after install.

`Scan Values` is read-only.

The Bit Wrecked tool can also remove the mod, open your `Mods` folder, or close without changing anything.

If you used `Brutal Science`, the `Restore Cap` button can restore `serverconfig.xml` from the newest Bit Wrecked animal-cap backup beside `serverconfig.xml`.
...
`Remove Mod` removes the Bit Wrecked modlet folder. It does not silently change `serverconfig.xml`.

To undo Brutal Science, use `Restore Cap`. The tool restores `serverconfig.xml` from the newest Bit Wrecked animal-cap backup, then verifies `MaxSpawnedAnimals`.
