...
Default means "match the live game baseline." It does not mean "no animals." For a true untouched vanilla state, remove the mod. For a baseline installed state, use Default.
...
`Restore Cap` restores `serverconfig.xml` from the newest Bit Wrecked animal-cap backup and verifies `MaxSpawnedAnimals` afterward.
...
- unchecked animals mean no generated change for that animal
- Default slider values mean live game baseline values
- Remove Mod means a true untouched vanilla state
...
The tool includes an explicit Brutal Science checkbox instead of changing the cap silently. It warns the user, backs up `serverconfig.xml`, verifies the new value, and reports whether the cap changed or was already lifted. `Restore Cap` restores from the newest Bit Wrecked backup and verifies the restored value.
