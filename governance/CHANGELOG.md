# Governance And Infrastructure Changelog

This changelog records repository-governance, evidence, build-pipeline, and maintainer-infrastructure changes. Player-visible mod behavior and release changes belong in the applicable solution changelog.
...
- Provisionally accepted a nine-file source-form primary ZIP allowlist with a copyleft-completeness gate: retain editable BAT/PowerShell/XML source, full GPL text, notices, change history, exact source provenance, and downstream freedoms while blocking the GUI avatar on explicit ownership/license evidence.
- Recorded the owner's attestation that Bit Wrecked owns the exact GUI avatar, fingerprinted the current and historical byte-identical PNG, and opened its copyright-license decision without treating Git or YouTube presence as a license grant.
- Removed the optional avatar from the planned `4.1.0` primary ZIP allowlist for cross-domain portability, reducing the customer contract to eight files while preserving repository source, historical artifacts, and the still-open broader identity boundary.
- Preserved Bit Wrecked authorship, UI text, stable mod/install/backup identifiers, and official provenance for the frozen `4.1.0` baseline while deferring configurable or white-label branding as future feature work.
- Established a user-first public-surface hierarchy, consolidating plain-language GPL/source/no-warranty/origin guidance into the primary README while keeping full legal text, source notices, and maintainer legal/use documents in their appropriate layers.
- Set the primary README and changelog themselves to `GPL-3.0-or-later`, preserving editable documentation source, explicit notices, downstream reuse, and a narrow scope that does not silently license unrelated governance files.
- Resolved future artifact retention: keep new ZIPs out of Git history, promote only validated upload files through an ignored versioned `final-upload` stage, attach them to GitHub Releases, and commit their checksums, inventories, provenance, and publication references.
- Closed the owner interview at the primary-baseline shape threshold, moved reversible release mechanics back to engineering defaults, and retained explicit escalation only for material product, rights, identity, publication, irreversible-state, or risk decisions.
- Added a lightweight execution planner with one active phase, explicit pass gates, a concise blocker register, and next-AI routing; the live session plan is the sandboxed working state and the governance file is its durable mirror.
- Captured the 32-file owner-approved solution baseline as raw-Git-blob SHA-256 evidence and reconciled product identity, document authority, edition state, and known contradictions into one current truth map before solution-facing preparation.
- Added a schema-validated per-solution release manifest, exact eight-file primary allowlist, raw-blob baseline and technical-freeze checks, protected historical-artifact hashes, and a read-only-by-default maintainer validation entry point.
- Established ignored, clean-`HEAD`-bound primary staging with atomic tree/evidence commit and no candidate build or final-upload promotion; blocked unsafe legacy rebuild destinations and kept no-scripts and Vortex machine-gated pending their independent evidence.
...
- Recorded owner acceptance of the P3 planning set and one technical-only P4 `PreparePrimary` authorization, froze the `4.1.0` candidate-source lifecycle and deterministic stored-ZIP contract, and retained all public actions as separate owner gates; no candidate has been built or promoted.
