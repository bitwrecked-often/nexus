...
The executable portion is readable source form. Q16 supersedes Q14's planned avatar inclusion and removes that optional decorative personalization from the future primary customer ZIP so the package can move more cleanly between domains. The avatar remains intact in repository history and the immutable `4.0.1` archive. Q18/Q19 resolve the primary README/changelog license scope and the plain-language license/use notice consolidation into `README_FIRST.txt`; implementation still waits for the fingerprint and preparation gates. The allowlist authorizes planning and validation rules, not a candidate build or a runtime change.
